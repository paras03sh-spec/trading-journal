...
function EODTab({data,onChange,trades,date,isMobile}){
  const set=k=>v=>onChange({...data,[k]:v});
  // Safe fallbacks for array fields — old saved data may not have them
  const safeData={
    narrativeReversalLevels:[],
    narrativeOrderFlow:[],
    narrativeProfile:'',
    narrativeDayChar:'',
    narrativeLead:'',
    well:'',fix:'',review:'',
    ...data,
    narrativeReversalLevels:Array.isArray(data?.narrativeReversalLevels)?d.narrativeReversalLevels:[],
    narrativeOrderFlow:Array.isArray(data?.narrativeOrderFlow)?d.narrativeOrderFlow:[],
  };
  const d=safeData;
  const total=trades.reduce((s,t)=>s+calcPnL(t.ticker,t.contracts,t.points),0);
  const esPts=trades.filter(t=>['ES','MES'].includes(t.ticker)).reduce((s,t)=>s+(parseFloat(t.points)||0),0);
...
Market Narrative: Profile=${d.narrativeProfile||'—'} | DayChar=${d.narrativeDayChar||'—'} | Led=${d.narrativeLead||'—'} | ReversalLevels=${(d.narrativeReversalLevels||[]).join(',')||'—'} | OrderFlow=${(d.narrativeOrderFlow||[]).join(',')||'—'}
    ...
    <div>
      ...
      <div style={{marginBottom:16}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>
          {[
            ...
            const active=d.narrativeProfile===o.value;
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>
          {[
            ...
            const active=d.narrativeDayChar===o.value;
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>
          {[
            ...
            const active=d.narrativeLead===o.value;
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>
          {[
            ...
            const cur=d.narrativeReversalLevels||[];
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          {[
            ...
            const cur=d.narrativeOrderFlow||[];
            const active=cur.includes(o.value);
      ...
      {d.well&&(
        <div style={{background:C.green+'10',border:`1px solid ${C.green}30`,borderRadius:12,padding:'14px 16px',marginBottom:12}}>
          ...
          <div style={{fontSize:13,color:C.text,lineHeight:1.7,whiteSpace:'pre-wrap'}}>{d.well}</div>
        </div>
      )}
      {d.fix&&(
        <div style={{background:C.red+'10',border:`1px solid ${C.red}30`,borderRadius:12,padding:'14px 16px',marginBottom:16}}>
          ...
          <div style={{fontSize:13,color:C.text,lineHeight:1.7,whiteSpace:'pre-wrap'}}>{d.fix}</div>
        </div>
      )}
