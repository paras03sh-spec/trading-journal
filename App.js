...
function emptyDay(){
  return{
    pre:{dailyBias:'',bigPicture:'',plan:'',feelings:'',
      esImgBalance:'',esImgTPO:'',esImg15:'',
      nqImgBalance:'',nqImgTPO:'',nqImg15:''},
    trades:[newTrade()],
    ...
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,marginBottom:10,overflow:'hidden'}}>
      <div onClick={()=>set('open')(!trade.open)} style={{padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:trade.open?C.surface2:C.surface}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          {trade.plan&&<span style={{fontSize:11,color:C.textMut}}>{({balance:'balance trade',failedexp:'failed exp',reclaim:'reclaim',breakout:'breakout'})[trade.plan]||trade.plan}</span>}
        </div>
...
// ─── Pre-Market Tab — 6 images: ES (Balance, TPO, 15min) + NQ (Balance, TPO, 15min) ──
  ...
  return(
    <div>
      ...
      <div style={{display:isMobile?'block':'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:20}}>
        <ImageSlot label="Balance" value={data.esImgBalance} onChange={set('esImgBalance')} accent={C.blue}/>
      ...
      <div style={{display:isMobile?'block':'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:8}}>
        <ImageSlot label="Balance" value={data.nqImgBalance} onChange={set('nqImgBalance')} accent={C.purple}/>
